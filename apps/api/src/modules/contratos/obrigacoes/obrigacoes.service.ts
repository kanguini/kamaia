...
export class ContratoObrigacoesService {
  ...
  async cumprirInstancia(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const u = await tx.contratoObrigacaoInstancia.update({
      ...
      if (instancia.obrigacao.periodicidade !== ObrigacaoPeriodicidade.UNICA) {
        const proxima = nextDate(dataReal, instancia.obrigacao.periodicidade as ObrigacaoPeriodicidade);
        await tx.contratoObrigacaoInstancia.create({
          data: {
...
function nextDate(from: Date, p: ObrigacaoPeriodicidade): Date {
  ...
  switch (p) {
    case ObrigacaoPeriodicidade.MENSAL:
      d.setMonth(d.getMonth() + 1);
      break;
    case ObrigacaoPeriodicidade.BIMESTRAL:
      d.setMonth(d.getMonth() + 2);
      break;
    case ObrigacaoPeriodicidade.TRIMESTRAL:
      d.setMonth(d.getMonth() + 3);
      break;
    case ObrigacaoPeriodicidade.SEMESTRAL:
