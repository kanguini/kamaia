...
export function AgendaEventFormModal({ open, onClose, onSuccess }: AgendaEventFormModalProps) {
  ...
  const { mutate: createEvent, loading, error } = useMutation<CreateEventData, { id: string }>(
    '/calendar/events',
    'POST'
  )
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Novo Evento" size="lg">
      {error && (
