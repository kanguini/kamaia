...
interface LogoProps {
  className?: string
  /** Height in pixels. Width auto-scales based on 225.83:35.81 ratio. */
  height?: number
}

/**
 * Kamaia official wordmark logo.
 * Uses currentColor so it adapts to the parent text color (supports dark mode).
 *
 * @example
 * <div className="text-ink"><Logo height={36} /></div>  // Uses ink color
 * <div className="text-white"><Logo height={28} /></div> // White logo
 */
...
/**
 * Compact icon version — first "K" glyph only, for tight spaces (favicon, avatar).
 */
export function LogoIcon({ className, size = 32 }: { className?: string; size?: number }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 42.27 35.81"
      width={size}
      height={size}
      aria-label="Kamaia"
      role="img"
